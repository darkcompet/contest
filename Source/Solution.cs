...
public class Solution : BaseSolution {
	...
	public int CountMatchingSubarrays(int[] nums, int[] pattern) {
		var N = nums.Length;
		var sign = new char[N - 1];
		for (var index = N - 2; index >= 0; --index) {
			sign[index] = "abc"[int.Sign(nums[index + 1] - nums[index]) + 1];
		}
		var pat = new char[pattern.Length];
		for (var index = pattern.Length - 1; index >= 0; --index) {
			pat[index] = "abc"[pattern[index] + 1];
		}
		var s = new string(sign);
		var p = new string(pat);
		return new KMP().Search(s, p).Count;
	}
}
