...
/// <summary>
/// This is heap, or priority queue as another name.
/// It performs some fast actions:
/// - FindMin()/FindMax(): O(1)
/// - Enqueue()/Dequeue(): O(log(N))
/// </summary>
/// <typeparam name="TElement"></typeparam>
/// <typeparam name="TPriority"></typeparam>
public class Heap<TElement, TPriority> {
	/// <summary>
	...
	/// <summary>
	///  Initializes a new instance of the <see cref="Heap{TElement, TPriority}"/> class.
	/// </summary>
	public Heap() {
	...
	/// </exception>
	public Heap(int initialCapacity) : this(initialCapacity, comparer: null) {
	}
	...
	/// </param>
	public Heap(IComparer<TPriority>? comparer) {
		_nodes = Array.Empty<(TElement, TPriority)>();
	...
	///  The specified <paramref name="initialCapacity"/> was negative.
	/// </exception>
	public Heap(int initialCapacity, IComparer<TPriority>? comparer) {
	...
	/// </remarks>
	public Heap(IEnumerable<(TElement Element, TPriority Priority)> items)
	...
	///  which is generally faster than enqueuing individual elements sequentially.
	/// </remarks>
	public Heap(IEnumerable<(TElement Element, TPriority Priority)> items, IComparer<TPriority>? comparer) {
	...
	/// <summary>
	///  Gets the number of elements contained in the <see cref="Heap{TElement, TPriority}"/>.
	/// </summary>
	public int Count => _size;

	/// <summary>
	///  Gets the priority comparer used by the <see cref="Heap{TElement, TPriority}"/>.
	/// </summary>
	...
	/// <summary>
	///  Adds the specified element with associated priority to the <see cref="Heap{TElement, TPriority}"/>.
	/// </summary>
	/// <param name="element">The element to add to the <see cref="Heap{TElement, TPriority}"/>.</param>
	...
	/// <summary>
	///  Returns the minimal element from the <see cref="Heap{TElement, TPriority}"/> without removing it.
	/// </summary>
	/// <exception cref="InvalidOperationException">The <see cref="Heap{TElement, TPriority}"/> is empty.</exception>
	/// <returns>The minimal element of the <see cref="Heap{TElement, TPriority}"/>.</returns>
	...
	/// <summary>
	///  Removes and returns the minimal element from the <see cref="Heap{TElement, TPriority}"/>.
	/// </summary>
	...
	/// <summary>
	///  Removes the minimal element and then immediately adds the specified element with associated priority to the <see cref="Heap{TElement, TPriority}"/>,
	/// </summary>
	/// <param name="element">The element to add to the <see cref="Heap{TElement, TPriority}"/>.</param>
	/// <param name="priority">The priority with which to associate the new element.</param>
	/// <exception cref="InvalidOperationException">The queue is empty.</exception>
	...
	/// <summary>
	///  Removes the minimal element from the <see cref="Heap{TElement, TPriority}"/>,
	...
	///  <see langword="false"/> if the <see cref="Heap{TElement, TPriority}"/> is empty.
	/// </returns>
	...
	/// <summary>
	///  Returns a value that indicates whether there is a minimal element in the <see cref="Heap{TElement, TPriority}"/>,
	///  and if one is present, copies it to the <paramref name="element"/> parameter,
	///  and its associated priority to the <paramref name="priority"/> parameter.
	///  The element is not removed from the <see cref="Heap{TElement, TPriority}"/>.
	/// </summary>
	...
	/// <summary>
	///  Enqueues a sequence of element/priority pairs to the <see cref="Heap{TElement, TPriority}"/>.
	/// </summary>
	...
	/// <summary>
	///  Enqueues a sequence of elements pairs to the <see cref="Heap{TElement, TPriority}"/>,
	...
	/// <summary>
	///  Removes all items from the <see cref="Heap{TElement, TPriority}"/>.
	/// </summary>
	public void Clear() {
	...
	/// <summary>
	///  Ensures that the <see cref="Heap{TElement, TPriority}"/> can hold up to
	...
	/// </exception>
	/// <returns>The current capacity of the <see cref="Heap{TElement, TPriority}"/>.</returns>
	...
	/// <summary>
	///  Sets the capacity to the actual number of items in the <see cref="Heap{TElement, TPriority}"/>,
	...
	/// <summary>
	///  Enumerates the contents of a <see cref="Heap{TElement, TPriority}"/>, without any ordering guarantees.
	/// </summary>
	...
	public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection {
		internal readonly Heap<TElement, TPriority> _queue;

		internal UnorderedItemsCollection(Heap<TElement, TPriority> queue) => _queue = queue;
		...
		/// <summary>
		///  Enumerates the element and priority pairs of a <see cref="Heap{TElement, TPriority}"/>,
		///  without any ordering guarantees.
		/// </summary>
		public struct Enumerator : IEnumerator<(TElement Element, TPriority Priority)> {
			private readonly Heap<TElement, TPriority> _queue;
			private readonly int _version;
			private int _index;
			private (TElement, TPriority) _current;

			internal Enumerator(Heap<TElement, TPriority> queue) {
				_queue = queue;
				_index = 0;
		...
		IEnumerator<(TElement Element, TPriority Priority)> IEnumerable<(TElement Element, TPriority Priority)>.GetEnumerator() =>
				_queue.Count == 0 ?
				((IEnumerable<(TElement Element, TPriority Priority)>)Array.Empty<(TElement Element, TPriority Priority)>()).GetEnumerator() :
				GetEnumerator();
